...
if (!$id) {
    ...
    $item = select_sql("SELECT * FROM carousel2 WHERE id = ?", [$id])[0] ?? null;
    if (!$item) {
...
// Pegar apenas o nome do arquivo e adicionar o prefixo correto
    ...
    if ($id) {
        idu_sql(
            "UPDATE carousel2 SET titulo = ?, texto = ?, data = ?, imagem = ? WHERE id = ?",
            [$novoTitulo, $novoTexto, $novoTexto2, $novaImagem, $id]
        );

        // Atualizar título e URL na navbar correspondente
        $id_navbar = select_sql("SELECT pagina_url FROM carousel2 WHERE id = ?", [$id])[0]['pagina_url'];
        $url = "destaque.php?id=$id_navbar";
        ...
        $paiSolucoes = select_sql("SELECT id FROM navbar WHERE titulo = 'Destaques' LIMIT 1")[0]['id'] ?? null;
        if (!$paiSolucoes) die("Erro: submenu 'Destaques' não encontrado!");
        ...
        // Inserir página
        $consulta2 = $pdo->prepare("INSERT INTO carousel2 (titulo, texto, data, imagem) VALUES (?, ?, ?, ?)");
...
<div class="caixa">
    ...
    <form method="post">
        ...
        <a href="editar_destaques.php" class="btn btn-secondary">Cancelar</a>
    </form>
</div>
