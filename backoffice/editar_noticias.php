...
$headerSolucoes = select_sql("SELECT * FROM headers WHERE tipo_pagina = 'noticias e eventos' LIMIT 1")[0] ?? null;
...
if (isset($_POST['guardar_banner'])) {
    ...
    if ($headerSolucoes) {
        ...
        idu_sql("INSERT INTO headers (tipo_pagina, imagem, ativo, ordem)VALUES ('noticias e eventos', ?, 1, 1)",[$novoBanner]);
    }
    $_SESSION['mensagem_sucesso'] = "Banner de Noticias e Eventos atualizado com sucesso!";
...
if(isset($_POST['toggle_id'])) {
    ...
    $_SESSION['mensagem_sucesso'] = $ativo ? "Noticia e Envento ativado!" : "Noticia e Envento desativado!";
    header("Location: editar_noticias.php");
    exit;
}
/* ====== Eliminar página ====== */
if (isset($_POST['delete_id'])) {
    ...
    $id_navbar = select_sql("SELECT pagina_url FROM footer_carousel WHERE id = ?",[$idEliminar])[0]['id_navbar'] ?? null;
...
$paginas = select_sql("SELECT * FROM footer_carousel ORDER BY id");
...
<div class="caixa">
    ...
    <table class="table table-bordered align-middle text-start">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <a href="editar_conteudo_noticias.php?id=<?= $p['id'] ?>" class="btn btn-dark btn-sm">
                        Editar
                    </a>
    ...
    <div class="mb-3">
        <a href="editar_conteudo_noticias.php" class="btn btn-dark">
